# GD[N] Research Guide for Claude
...
Each Global Dialogue focuses on specific themes related to artificial intelligence, exploring public perspectives, concerns, and preferences regarding AI development and deployment.

## Research Task

Your task is to answer the investigation questions located in `analysis_output/GD[N]/research/GD[N]_investigation_questions.md` one at a time, following the guidelines in this guide, and document your answers rigorously and thoroughly according to the guidance in this document in `analysis_output/GD[N]/research/GD[N]_investigation_answers.md`, using the connection to the Sqlite db to run queries on the GD[N] data (refer to the `docs/database_guide.md`) and any Python scripts needed to complete the analysis. After documenting your answer, commit your work to git with a succinct message clearly indicating the investigation question number answered.
...
```bash
# Connect to database
make db-connect GD=[N]

# Or directly
sqlite3 Data/GD[N]/GD[N].db
```

### Key Files
- **Investigation Questions**: `analysis_output/GD[N]/research/GD[N]_investigation_questions.md`
- **Survey Structure**: `Data/GD[N]/GD[N]_survey_human_readable.md`
- **Database Guide**: `docs/database_guide.md`

## Database Schema

### Core Tables
- **`responses`**: All participant responses (varies by dialogue)
...
conn = sqlite3.connect('Data/GD[N]/GD[N].db')
```
...
For composite scores (example with loneliness score):
```sql
-- Reverse score positive items as needed
-- Higher score = more of the measured construct
...
```sql
-- Example: Identify participants with specific behavior patterns
SELECT participant_id, COUNT(DISTINCT question_id) as activity_count
FROM responses
WHERE question_id IN ('uuid1', 'uuid2', 'uuid3')  -- Relevant questions
  AND response = 'Yes'
GROUP BY participant_id
HAVING activity_count >= 4;
```
...
3. **Create composite scores**: Build relevant composite measures based on the dialogue's focus
...
- Narrative insights about the dialogue's themes

## Documentation Format

### Investigation Answer Structure
Each answer in `GD[N]_investigation_answers.md` should follow this format:
...
**Insights:** [Narrative interpretation of what this reveals about the dialogue's themes]
...
- Replace [N] with the actual Global Dialogue number (e.g., GD1, GD2, GD3, GD4, GD5)
