...
write_output("**Finding:** What is the demographic breakdown of the survey respondents based on age (Q2), gender (Q3), location type (Q4), and religious identification (Q6)?")
...
WHERE r.question LIKE '%How old are you?%'
  AND p.pri_score >= 0.3
GROUP BY r.response
ORDER BY 
  CASE r.response
...
FROM responses r
JOIN participants p ON r.participant_id = p.participant_id
WHERE r.question LIKE '%What is your gender?%'
  AND p.pri_score >= 0.3
GROUP BY r.response
ORDER BY count DESC
"""
...
for _, row in gender_df.iterrows():
    pct = (row['count'] / total_gender) * 100
...
FROM responses r
JOIN participants p ON r.participant_id = p.participant_id
WHERE r.question LIKE '%What best describes where you live?%'
  AND p.pri_score >= 0.3
GROUP BY r.response
ORDER BY count DESC
"""
...
for _, row in location_df.iterrows():
    pct = (row['count'] / total_location) * 100
...
FROM responses r
JOIN participants p ON r.participant_id = p.participant_id
WHERE r.question LIKE '%What religious group or faith do you most identify with?%'
  AND p.pri_score >= 0.3
GROUP BY r.response
ORDER BY count DESC
"""
...
for _, row in religion_df.iterrows():
    pct = (row['count'] / total_religion) * 100
...
write_output("**Finding:** What is the distribution of views on the relationship between humans and nature (Q31)? How does this distribution vary across different religious groups (Q6) or residential environments (Q4)?")
...
WHERE r1.question = 'Which statement best reflects your view?'
  ...
  AND r2.question LIKE '%What religious group or faith do you most identify with?%'
  AND p.pri_score >= 0.3
...
write_output("**Human-Nature Views by Religion:**")
for religion in religion_nature_df['religion'].unique():
    write_output(f"\n{religion}:")
...
WHERE r1.question = 'Which statement best reflects your view?'
  AND r1.response LIKE '%nature%'
  AND r2.question LIKE '%What best describes where you live?%'
  AND p.pri_score >= 0.3
GROUP BY r2.response, r1.response
ORDER BY r2.response, count DESC
"""
...
write_output("**Finding:** What percentage of respondents believe humans are superior, inferior, or equal to animals (Q32)? Is there a significant correlation between this belief and a respondent's age (Q2), religious affiliation (Q6), or level of interaction with animals (Q35-Q38)?")
...
for _, row in superiority_df.iterrows():
    pct = (row['count'] / total_superiority) * 100
    write_output(f"- {row['view']}: {row['count']} ({pct:.1f}%)")
write_output("")
...
write_output("**Finding:** What is the overall public sentiment towards the increased use of AI in daily life (Q5)? How does this sentiment (excited vs. concerned) correlate with daily, weekly, or monthly personal use of AI (Q20)?")
...
FROM responses r
JOIN participants p ON r.participant_id = p.participant_id
WHERE r.question LIKE '%Overall, would you say the increased use of artificial intelligence%'
  AND p.pri_score >= 0.3
GROUP BY r.response
ORDER BY count DESC
"""
...
for _, row in ai_sentiment_df.iterrows():
    pct = (row['count'] / total_sentiment) * 100
...
JOIN participants p ON r1.participant_id = p.participant_id
JOIN responses r2 ON r1.participant_id = r2.participant_id
WHERE r1.question LIKE '%Overall, would you say the increased use of artificial intelligence%'
  AND r2.question LIKE '%how often, if at all, have you personally chosen to use an AI system in your personal life%'
  AND p.pri_score >= 0.3
...
write_output("**Finding:** How does trust in AI chatbots (Q17) compare to trust in other entities like family doctors (Q12), elected representatives (Q14), and social media feeds (Q13)? What is the average trust level for each entity?")
...
trust_entities = [
    ('family doctor', '%trust your family doctor%'),
    ('social media feed', '%trust your social media feed%'),
    ('elected representatives', '%trust your elected representatives%'),
    ('faith or community leader', '%trust your faith or community leader%'),
    ('civil servants', '%trust the civil servants%'),
    ('AI chatbot', '%trust your AI chatbot%')
]
...
trust_scores = {}
for entity_name, entity_query_pattern in trust_entities:
    trust_query = f"""
    SELECT 
        ...
        CASE 
            ...
            WHEN r.response = 'Neither Trust Nor Distrust' OR r.response = 'Neither Trust nor Distrust' THEN 3
            WHEN r.response = 'Somewhat Distrust' THEN 2
            WHEN r.response = 'Strongly Distrust' THEN 1
        END as trust_score
    FROM responses r
    JOIN participants p ON r.participant_id = p.participant_id
    WHERE r.question LIKE '{entity_query_pattern}'
      AND p.pri_score >= 0.3
    GROUP BY r.response
    """
...
# Statistical comparison between AI chatbot and other entities
ai_trust_comparison_query = """
SELECT 
    r1.participant_id,
    CASE 
        WHEN r1.response = 'Strongly Trust' THEN 5
        WHEN r1.response = 'Somewhat Trust' THEN 4
        WHEN r1.response = 'Neither Trust Nor Distrust' OR r1.response = 'Neither Trust nor Distrust' THEN 3
    ...
    CASE 
        ...
        WHEN r2.response = 'Neither Trust Nor Distrust' OR r2.response = 'Neither Trust nor Distrust' THEN 3
...
JOIN participants p ON r1.participant_id = p.participant_id
JOIN responses r2 ON r1.participant_id = r2.participant_id
WHERE r1.question LIKE '%trust your AI chatbot%'
  AND r2.question LIKE '%trust your family doctor%'
  AND p.pri_score >= 0.3
"""
trust_comparison_df = pd.read_sql_query(ai_trust_comparison_query, conn)
...
write_output("3. The majority of respondents view humans as either superior or equal to animals, with significant age-related patterns")
write_output("4. AI sentiment is mixed, with personal usage strongly correlating with positive sentiment")
write_output("5. Trust in AI chatbots ranks lower than traditional authorities like family doctors but higher than social media")
write_output("")

write_output("## SQL Queries Used")
write_output("")
write_output("All queries shown in the analysis above were used to generate the findings.")
write_output("")
