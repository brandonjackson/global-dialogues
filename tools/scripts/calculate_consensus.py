...
from lib.analysis_utils import parse_percentage, get_segment_columns, parse_gd_identifier, validate_gd_directory
...
def main():
    ...
    if args.gd_number:
        ...
        if not os.path.exists(counts_csv_path):
            parser.error(f"Segment counts input file not found for {gd_identifier}. Expected at: {counts_csv_path}")
